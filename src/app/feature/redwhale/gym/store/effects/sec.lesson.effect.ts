...
export class LessongEffect {
    ...
    public loadLessonCategs$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({ gymId }) =>
                this.gymLessonApi.getCategoryList(gymId).pipe(
                    map((categs) => {
    ...
    public addLessonCateg = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({ gymId, categName }) =>
                this.gymLessonApi.createCategory(gymId, { name: categName }).pipe(
        ...
        () =>
            this.actions$.pipe(
                ...
                switchMap(({ gymId, id, categName }) =>
                    this.gymLessonApi.updateCategory(gymId, id, { name: categName }).pipe(
                        tap(),
                        catchError((err: string) => of(LessonActions.error({ error: err })))
                    )
                )
    ...
    public updateCategLesson = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({ gymId, categId, categName, reqBody }) =>
                this.gymLessonApi.createItem(gymId, categId, reqBody).pipe(
                    map((newItem) =>
    ...
    public getTrainerFilterList = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({ gymId }) => {
                return this.gymUserApi.getUserList(gymId, '', 'administrator, manager, staff').pipe(
                    map((managers) => {
        ...
        this.actions$.pipe(
            ...
            switchMap(({ selectedLesson, reqBody }) => {
                ...
                return this.gymLessonApi
                    .updateItem(selectedLesson.gymId, selectedLesson.categId, selectedLesson.lessonData.id, reqBody)
                    .pipe(
                        ...
                        switchMap(([action, lesCategEn]) =>
                            this.gymLessonApi.getCategoryList(selectedLesson.gymId).pipe(
                                map((categs) => {
                                    const categState = _.map(categs, (categ) => {
                                ...
                                concatLatestFrom(() => this.store.select(LessonSelector.currentGym)),
                                switchMap(([lesCategState, curGym]) => {
                                    return [
                                        LessonActions.updateLessonCategs({
                                            lessonCategState: lesCategState,
                                        }),
                                        MembershipActions.startUpsertState({ gymId: curGym }),
                                    ]
                                })
                            )
                        )
    ...
    public removeSelectedLesson$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({ selectedLesson }) =>
                this.gymLessonApi
                    .deleteItem(selectedLesson.gymId, selectedLesson.categId, selectedLesson.lessonData.id)
                    .pipe(
                        catchError((err: string) => of(LessonActions.error({ error: err }))),
                        map(() => MembershipActions.startUpsertState({ gymId: selectedLesson.gymId }))
                    )
            )
        )
    )
    ...
    public refreshSelectedLesson$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(([action, selectedLesson]) =>
                this.gymLessonApi
                    .getItem(selectedLesson.gymId, selectedLesson.categId, selectedLesson.lessonData.id)
                    .pipe(
                        map((lessonItem) => {
    ...
    public upsertState$ = createEffect(() =>
        this.actions$.pipe(
            ...
            concatLatestFrom(() => this.store.select(LessonSelector.LessonCategEntities)),
            switchMap(([action, lesCategEn]) =>
                this.gymLessonApi.getCategoryList(action.gymId).pipe(
                    map((categs) => {
                        const categState = _.map(categs, (categ) => {
                            const _categState: LessonCategoryState = {
                                ...categ,
