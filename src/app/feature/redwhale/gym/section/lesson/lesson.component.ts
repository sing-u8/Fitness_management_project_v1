...
} from '@gymStore/reducers/sec.lesson.reducer'
...
export class LessonComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    public gymMembershipCategList: Array<MembershipCategory>
    public gymReservableMembershipItemList: Array<MembershipItem>
    ...
    constructor(
        ...
        this.nxStore.pipe(select(LessonSelector.currentGym), takeUntil(this.unSubscriber$)).subscribe((curGym) => {
            if (curGym != this.gym.id) {
                ...
                this.nxStore.dispatch(LessonActions.startLoadLessonCategs({ gymId: this.gym.id }))
            }
        })

        this.nxStore.dispatch(LessonActions.setCurrentGym({ currentGym: this.gym.id }))
        this.nxStore.dispatch(LessonActions.startGetTrainerFilterList({ gymId: this.gym.id }))

        this.gymMembershipService.getCategoryList(this.gym.id).subscribe((categs) => {
            this.isReserveMembershipExist = categs.some((v) => {
                return v.items.length > 0
            })
            this.gymMembershipCategList = categs
    ...
    addCategory() {
        if (this.addCategForm.value) {
            this.nxStore.dispatch(
                LessonActions.startAddLessonCateg({ gymId: this.gym.id, categName: this.addCategForm.value })
            )
        }
        this.addCategOff()
    ...
    initAddReservableMembershipList(lesItem: FromLesson.SelectedLesson) {
        this.gymReservableMembershipItemList = _.reduce(
            _.cloneDeep(this.gymMembershipCategList),
            (list, v) => {
        ...
        _.remove(this.gymReservableMembershipItemList, (v) => {
    ...
    updateSelLesson(selectedLesson: SelectedLesson, reqBody: UpdateItemRequestBody) {
        ...
        // this.nxStore.dispatch(MembershipActions.startUpsertState({ gymId: this.gym.id }))
    }
